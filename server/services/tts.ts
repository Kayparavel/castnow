...
function loadVoiceSample(): string {
  ...
  const resolved = samplePath.startsWith(".") ? join(import.meta.dirname, "../../", samplePath) : samplePath
  const buf = readFileSync(resolved)
  const ext = resolved.split(".").pop()?.toLowerCase()
  if (ext === "mp3") voiceMime = "audio/mpeg"
  else voiceMime = "audio/wav"
